...
export default function CartPage() {
  ...
  const getDuplicateItems = () => {
    ...
    let numberArray: any = [];
    for (var i = 0; i < stringCartKeys.length; i++)
      numberArray.push(parseInt(stringCartKeys[i]));
    ...
    setCartToMap(
      arrayOfOrders.map((item: any, index: number) => ({
        ...item,
        quantity: Object.values(cartItemWithCount)[index],
      }))
    );
  };
